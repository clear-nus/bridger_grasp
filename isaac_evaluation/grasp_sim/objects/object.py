...
class SimpleObject():
    ...
    def __init__(self, gym, sim, env, isaac_base, env_number, pose, obj_id, obj_name, obj_type='Mug',
        ...
        if obj_type == 'Fork':
            self.gym.set_actor_scale(self.env, self.handle, scale * 2.0)
        elif obj_type == 'MousePad':
            self.gym.set_actor_scale(self.env, self.handle, scale * 1.5)
        else:
            self.gym.set_actor_scale(self.env, self.handle, scale * 1.0)

        # self.gym.set_actor_scale(self.env, self.handle, scale*5)
        # print(self.gym.get_actor_scale(self.env, self.handle))
    ...
    def _set_assets(self):
        ...
        # asset_options.thickness = 0.0  # TODO: changed this
