...
class ApproximatedGrasp_AnnealedLD():
    ...
    def _step(self, H0, t, task_id=None, noise_off=True):
        ...
        if task_id is not None:
            energy = self.model(H_in, t_in, task_id)
        else:
...
class Grasp_AnnealedLD():
    ...
    def _step(self, H0, t, task_id=None, noise_off=True):
        ...
        for k in range(self.k_steps):
            ...
            if task_id is not None:
                e = self.model(H_in, t_in, task_id)
            else:
            ...
            weight = 1.0 # 1 * (self.k_steps - k * 1.0) / self.k_steps
            # print(t)
            delta = - weight * c_lr/2*d_phi + np.sqrt(c_lr)*noise
    ...
    def sample(self, save_path=False, batch=None, task_id=None):
        ## 1.Sample initial SE(3) ##
        if batch is None:
            batch = self.batch
        H0 = SO3_R3().sample(batch).to(self.device, torch.float32)
        ...
        if save_path:
            trj_H = Ht[None,...]
        for t in range(self.T):
            Ht = self._step(Ht, t, task_id=task_id, noise_off=self.deterministic)
            if save_path:
                trj_H = torch.cat((trj_H, Ht[None,:]), 0)
        for t in range(self.T_fit):
            Ht = self._step(Ht, self.T,  task_id=task_id, noise_off=True)
            if save_path:
                trj_H = torch.cat((trj_H, Ht[None,:]), 0)
