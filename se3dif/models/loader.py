...
def load_model(args):
    ...
    elif args['NetworkArch'] == 'PointcloudResidual':
        model = load_pointcloud_residual(args)
...
def load_pointcloud_residual(args):
    device = args['device']
    params = args['NetworkSpecs']
    feat_enc_params = params['feature_encoder']
    v_enc_params = params['encoder']
...
def load_pointcloud_si(args):
    ...
    model = models.SIFields(vision_encoder=vision_encoder, feature_encoder=feature_encoder, geometry_encoder=geometry_encoder,
                            v_decoder=v_decoder, s_decoder=s_decoder, b_decoder=b_decoder, points=points).to(device)
    return model
