...
class TGGraspDiffusionFields(nn.Module):
    ...
    def __init__(self, vision_encoder, geometry_encoder, points, feature_encoder, decoder, task_encoder):
        super().__init__()
    ...
    def forward(self, H, k, task_id):
        ...
        e = self.decoder(torch.cat([psi_flatten, task_embed], dim=-1))
        return e
...
class TGSIFields(nn.Module):
    ''' Grasp DiffusionFields. SE(3) diffusion model to learn 6D grasp distributions. See
        SE(3)-DiffusionFields: Learning cost functions for joint grasp and motion optimization through diffusion
    '''

    def __init__(self, vision_encoder, geometry_encoder, points, feature_encoder, v_decoder, s_decoder, b_decoder, task_encoder):
        super().__init__()
        ## Register points to map H to points ##
        self.register_buffer('points', points)
        ## Vision Encoder. Map observation to visual latent code ##
    ...
    def forward(self, H, k, task_id, decoder_type='all'):
        ## 1. Represent H with points
        p = self.geometry_encoder(H, self.points)
        k_ext = k.unsqueeze(1).repeat(1, p.shape[1])
        z_ext = self.z.unsqueeze(1).repeat(1, p.shape[1], 1)
        ...
        if decoder_type == 'all':
            v = self.v_decoder(torch.cat([psi_flatten, task_embed], dim=-1))
            s = self.s_decoder(torch.cat([psi_flatten, task_embed], dim=-1))
            b = self.b_decoder(torch.cat([psi_flatten, task_embed], dim=-1))
