...
def visualize_grasps(Hs, scale=1., p_cloud=None, energies=None, colors=None, mesh=None, show=True):
    ...
    def look_at(camera_position, camera_target, up_vector):
        ...
        return np.array([
            [vector2[0], vector3[0], vector[0], camera_position[0]],
            [vector2[1], vector3[1], vector[1], camera_position[1]],
            [vector2[2], vector3[2], vector[2], camera_position[2]],
            [0, 0, 0, 1.0]
        ])
        # return np.array([
    ...
    target = Hs.mean(axis=0)[0:3, 3]
    # target = scene.camera_transform[0:3, 3]
    eye = np.array([
        target[0] - 0.6,
        target[1],
        target[2] + 0.3
    ...
    up = np.array([0, 0,  1])
    scene.camera_transform = look_at(eye, target, up)
    # scene.camera_transform = np.matmul(
